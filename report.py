...
for service_name in os.listdir(REPORT_PREFIX):
    ...
    for down_interval in down_time:
        start_time, end_time = [datetime.datetime.strptime(i, DATE_INPUT_FORMAT) for i in down_interval]
        data[1] += (end_time - start_time).seconds
        data[2] += [start_time.strftime(DATE_OUTPUT_FORMAT), end_time.strftime(DATE_OUTPUT_FORMAT)]
    data[3] = round((SECONDS_IN_DAY - data[1]) / SECONDS_IN_DAY * 100, 2)
    data[2] = "---" if not data[2] else "-".join(data[2])
